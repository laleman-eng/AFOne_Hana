...
TEnumMensajes = public enum( SBO_Terminado 
  ...
  TUserFieldFormRecord = public record
  private
  public
      ParentFormUid : String;
      ChildFormUid  : String;
      Table         : String;
      Field         : String;
  end;
...
private
   ...
   FCerrarCRForm : Boolean;
   method GetSBO_f : CSBOFunctions;
   method SetSBO_f(Value  : CSBOFunctions);
   method GetCerrarCRForm : Boolean;
   method GetRunningUnderSQLServer: Boolean;
public
   ...
   FCompany          : SAPbobsCOM.Company;
    ...
    UsrFldsFormActive  : Boolean:=false;
    UsrFldsFormUid     : String :='';
    ListFormsUserField : List<TUserFieldFormRecord>:=New List<TUserFieldFormRecord>;
    CrystalReportFileName : String:='';
    CRFormPreviewUID : String :='';
    Ser_ItemCode : String:='';
    Ser_Serie    : String:='';
    TcpPort : Integer := 30033; 
...
APP_MESSAGE      : array [0..1,0..87] of String := [["Se ha cerrado Business One...   Terminando AddOn."
                                                 ...
                                                 ,"El Activo debe ser depreciado hasta el periodo en que se da de baja."
                                                 ,"El Activo debe ser contabilizado hasta el periodo en que se da de baja."
   ...
   property ValidarCierre     : Boolean:=True;
   property RunningUnderSQLServer:  Boolean  read GetRunningUnderSQLServer;
   property CerrarCRForm      : Boolean read GetCerrarCRForm;
   property oCompany: SAPbobsCOM.Company read FCompany write FCompany;
...
method TGlobalVid.GetRunningUnderSQLServer: Boolean;
var
  sVersion : String;
begin
   //exit(GLOB_TipoBD = "SQLServer");
   sVersion := FCompany.DbServerType.ToString();
   exit (not sVersion.StartsWith('dst_HANADB'));
end;
...
begin
  FSBO_f := Value;
  FCerrarCRForm := False;
  if RunningUnderSQLServer then
  begin
    oRecordSet := SAPbobsCOM.Recordset(Value.Cmpny.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset));
    try
        oRecordSet.DoQuery("SELECT COUNT(*) Resultado FROM sys.columns WHERE Name = N'U_CerrarCR' and Object_ID = Object_ID(N'@VID_PPRDPA')");
        if System.Int32(oRecordSet.Fields.Item('Resultado').Value)>0 then
        begin
          oRecordSet.DoQuery("SELECT ISNULL(U_CerrarCR, 'Y') CerrarCR FROM [@VID_PPRDPA]");
          FCerrarCRForm := System.String(oRecordSet.Fields.Item('CerrarCR').Value)<>'N';
        end;
    finally
       FSBO_f._ReleaseCOMObject(oRecordSet);
    end;
  end;  
end;

method TGlobalVid.GetCerrarCRForm: Boolean;
begin
  Result := FCerrarCRForm;
end;
