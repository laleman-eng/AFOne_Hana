namespace AF_IFRS.FacturaCompra;
...
type
  TFacturaCompra = class(TvkBaseForm, IvkFormInterface)
  private
...
method TFacturaCompra.InitForm(uid: string; xmlPath: string; var application:SAPbouiCOM.Application; var company:SAPbobsCOM.Company; var SBOFunctions: VisualD.SBOFunctions.CSBOFunctions ; var _GlobalSettings:TGlobalVid): boolean;
var
...
begin
   ...
   try
      ...
      oItmRef := FSBOf.getFormItem(oForm, '10000330');

      // Flete
      oItm    := oForm.Items.Add('btn_AF', BoFormItemTypes.it_BUTTON);
      oItm.Visible := false;
      oItm.Left   := oItmRef.Left - oItmRef.Width - 5;
...
method   TFacturaCompra.FormEvent(FormUID: string; var pVal: SAPbouiCOM.ItemEvent; var BubbleEvent: Boolean);
var
   oForm : SAPbouiCOM.Form;
   oSql  : string;
begin
      ...
      if (oForm.Mode <> BoFormMode.fm_ADD_MODE) and (oForm.Mode <> BoFormMode.fm_FIND_MODE) and (oForm.Mode <> BoFormMode.fm_VIEW_MODE) then begin
         if (pVal.EventType = BoEventTypes.et_ITEM_PRESSED) and (pVal.ItemUID = 'btn_AF') and (pVal.BeforeAction = false) then begin
            ...
            oSql := iif(GlobalSettings.RunningUnderSQLServer, 
                        "Select docentry from OPCH where docnum = {0}", 
                        'Select "DocEntry" from OPCH where "DocNum" = {0}');
            oSql := string.Format(oSql, EditText(FSBOf.getFormItem(oForm, '8').Specific).Value);
...
method TFacturaCompra.FormDataEvent(var BusinessObjectInfo: BusinessObjectInfo; var BubbleEvent: boolean);
var
   DocEntry  : string;
   i,f       : integer;
begin
...
method  TFacturaCompra.SetFormAdquision(oDocEntry: string);
var
...
begin
      for i:=0 to self.ooForms.Count-1 do begin
         ...
         if (oFormAux.TypeEx = 'VID_AFADQ') then 
            if (TAdquisicionActivos(oFormIvk).Tipodoc =  'F')  and (TAdquisicionActivos(oFormIvk).DocEntry  = oDocEntry) then
               exit; 
      end;
      ...
      if(oFormIvk<>nil) then begin
         TAdquisicionActivos(oFormIvk).Tipodoc   := 'F';
